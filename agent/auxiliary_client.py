...
def _try_openrouter(explicit_api_key: str = None, model: str = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    or_key = explicit_api_key or get_openrouter_key()
    if not or_key:
        _mark_provider_unhealthy("openrouter", ttl=60)
        return None, None
...
def _describe_openrouter_unavailable() -> str:
    ...
    if not str(get_openrouter_key() or "").strip():
...
def _get_cached_client(
    ...
    if not effective_api_key:
        if normalized_provider == "openrouter":
            effective_api_key = get_openrouter_key()
        else:
    ...
    with _client_cache_lock:
        if cache_key in _client_cache:
            ...
            for stale_key, stale_entry in list(_client_cache.items()):
                if (
                    stale_key[0] == provider
                    and stale_key[1] == async_mode
                    and stale_key[2] == (base_url or "")
                    and stale_key[4] == (api_mode or "")
                    and stale_key[6] == is_vision
                ):
                    _force_close_async_httpx(stale_entry[0])
                    del _client_cache[stale_key]
