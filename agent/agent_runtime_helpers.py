...
def recover_with_credential_pool(
    ...
    *,
    ...
    if (
        (status_code in {402, 429} or effective_reason in {FailoverReason.billing, FailoverReason.rate_limit})
        and (getattr(agent, "provider", "") or "").strip().lower() == "openrouter"
        and base_url_host_matches(getattr(agent, "base_url", "") or "", "openrouter.ai")
    ):
        try:
            from providers.openrouter_manager import get_openrouter_key, rotate_openrouter_key

            old_key = get_openrouter_key()
            new_key = rotate_openrouter_key()
        except Exception:
            old_key = ""
            new_key = ""
        if new_key and new_key != old_key:
            ...
            if isinstance(getattr(agent, "_client_kwargs", None), dict):
                agent._client_kwargs["api_key"] = new_key
            try:
                close_fn = getattr(getattr(agent, "client", None), "close", None)
                if callable(close_fn):
                    close_fn()
            except Exception:
                pass
            agent.client = agent._create_openai_client(
                dict(getattr(agent, "_client_kwargs", {}) or {}),
                reason="openrouter_key_rotation",
                shared=True,
            )
            _ra().logger.info("OpenRouter key manager rotated after upstream %s", status_code)
            return True, False
